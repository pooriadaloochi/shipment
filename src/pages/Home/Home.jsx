...
const Home = () => {
  ...
  useEffect(() => {
    if (originFormData && destinationFormData && goodFormData && loadFormData)
  ...
  const handleChangeLoadVisible = () => {
    setLoadVisibleForm(!loadVisibleForm);
  };
  ...
  console.log(loadVisibleForm);
  const handlesubmitAllFormData = async () => {
    // setLoading(true);
    try {
      const {statusCode, data} = await HomeApi.addLoad();
      if (statusCode === 'Success') {
        handleNotification('success', 'موفق', '');
        console.log(data);
      }
              ...
              <Row className="centerMenu">
                ...
                <Col span={5}>
                  <Popover
                    ...
                    placement="bottomRight"
                    trigger={'click'}
                    visible={loadVisibleForm}
                    onVisibleChange={handleChangeLoadVisible}
                  >
                    <div className="centerMenu_origin">
                      <div className="centerMenu_origin_top">
                        <h5>بار</h5>
